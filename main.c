...
/*                                                                  */
/* Argument:                                                        */
/*   count          -- semaphore initial value                      */
/* Return value:                                                    */
/*   Semaphore's ID -- Success                                      */
/*   mtu_ERROR      -- Failure                                      */
/* ---------------------------------------------------------------- */
...
SEM_t SEMAPHORE_INIT (int count)
{
     ...
     NewSemaphore = (struct SEMAPHORE*)malloc(sizeof(struct SEMAPHORE));
	 if (NewSemaphore == NULL)
		 return (SEM_t)mtu_ERROR;

	 NewSemaphore->Magic = NextSemNumber++;
	 NewSemaphore->Count = count;

	 NewSemaphore->WaitingQ.Head = NULL;
	 NewSemaphore->WaitingQ.Tail = NULL;
...
/*                                                                  */
/* Argument:                                                        */
/*   SEM_t semaphore     -- semaphore to be destroyed               */
/* Return value:                                                    */
/*   mtu_NORMAL          -- Success                                 */
/*   mtu_ERROR           -- Failure                                 */
/* ---------------------------------------------------------------- */

int SEMAPHORE_DESTROY(SEM_t Id)
{
	 THREAD_t temp;

     if (Id->WaitingQ.Head != NULL)
		 return mtu_ERROR;	/* There are still threads waiting */
...
/*                                                                  */
/* Argument:                                                        */
/*   SEM_t semaphore     -- semaphore to receive the signal         */
/* Return value:                                                    */
/*   mtu_NORMAL          -- Success                                 */
/*   mtu_ERROR           -- Failure                                 */
/* ---------------------------------------------------------------- */
...
int SEMAPHORE_SIGNAL (SEM_t semaphore)
{
	...
	if (semaphore->WaitingQ.Head != NULL)
	{
		temp = (THREAD_t)THREAD_Remove(&(semaphore->WaitingQ));
		THREAD_RESUME(temp);
	}
...
/*                                                                  */
/* Argument:                                                        */
/*   SEM_t semaphore     -- semaphore to wait on                    */
/* Return value:                                                    */
/*   mtu_NORMAL          -- Success                                 */
/*   mtu_ERROR           -- Failure                                 */
/* ---------------------------------------------------------------- */
...
int SEMAPHORE_WAIT (SEM_t semaphore)
{
	...
	{
		THREAD_Append(&(semaphore->WaitingQ), (void*)Running);
